...
class Repository {
        ...
        if (params) {
            ...
            Object.keys(params).forEach(function (paramName) {
                if (paramName == "sort") {
                    ...
                    // todo add search key
                    Object.keys(model).forEach(function (cle){
                        if(cle == paramName)
            ...
            for(let object of objectsList){
                if(this.filterInformations(object,searchKeys)){
                    filteredAndSortedObjects.push(object);
                }
            }
            // todo sort
            for(let cptFirstValue = 0;cptFirstValue<filteredAndSortedObjects.length;cptFirstValue++){
                for(let cptSecondValue = 0;cptSecondValue<filteredAndSortedObjects.length;cptSecondValue++){
                    let compareValue = this.innerCompare(filteredAndSortedObjects[cptFirstValue],filteredAndSortedObjects[cptSecondValue]);
                    let pog = filteredAndSortedObjects[cptFirstValue]
                    if(sortKeys[0]){
                        switch(compareValue){
                            case 1:
                                let tempValue = filteredAndSortedObjects[cptFirstValue];
                                filteredAndSortedObjects[cptFirstValue] = filteredAndSortedObjects[cptSecondValue]
                                filteredAndSortedObjects[cptSecondValue] = tempValue;
                                break;
                        }
                    }else{
                        switch(compareValue){
    ...
    filterInformations(objectToFilter,tabSearchKeys){
        var isCorrect = true;
        for(let singleSearchkey of tabSearchKeys){
            if(!(this.valueMatch(objectToFilter[singleSearchkey.key],singleSearchkey.value)))
                isCorrect = false;
        }
        return isCorrect;
    }
